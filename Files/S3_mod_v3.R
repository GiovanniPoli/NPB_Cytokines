...
settings = c(lapply(1101:1200, function(x) list("seed_data" = as.numeric(x), "var_lv" = "low" ,"seed_chain"= 250000 + x, "min" = 0.25, "max"= 0.75, "pi" = "Unif")),
             lapply(1201:1300, function(x) list("seed_data" = as.numeric(x), "var_lv" = "mid" ,"seed_chain"= 260000 + x, "min" = 0.75, "max"= 1.50, "pi" = "Unif")),
             lapply(1301:1400, function(x) list("seed_data" = as.numeric(x), "var_lv" = "high","seed_chain"= 270000 + x, "min" = 1.50, "max"= 3.00, "pi" = "Unif")),
             lapply(1101:1200, function(x) list("seed_data" = as.numeric(x), "var_lv" = "low" ,"seed_chain"= 280000 + x, "min" = 0.25, "max"= 0.75, "pi" = "FDR")),
             lapply(1201:1300, function(x) list("seed_data" = as.numeric(x), "var_lv" = "mid" ,"seed_chain"= 290000 + x, "min" = 0.75, "max"= 1.50, "pi" = "FDR")),
             lapply(1301:1400, function(x) list("seed_data" = as.numeric(x), "var_lv" = "high","seed_chain"= 300000 + x, "min" = 1.50, "max"= 3.00, "pi" = "FDR")))
...
foreach(it = settings, .options.snow = opts, .packages = c("lme4","reshape2","sn")) %dopar%{
  ...
  SIMULATION = simulate_data_heteroscedastic_no_groups_skewed(sd_min = it$min, sd_max = it$max,
                                                              seed = it$seed_data, delta_par = 99/100)
  
  data = SIMULATION$simulated_data
  
  obj_name = paste0("sim_s4_singleton_",it$var_lv, "_",it$seed_data,".rds")
  ...
  saveRDS(list("PRIORS"          = PRIORS,
               ...
               "time"            = Sys.time() - t0),
          file = paste0("Scenario_III/Mod_v3_ExFDR/",it$var_lv,"/",obj_name))
    ...
    saveRDS(list("PRIORS"          = PRIORS,
                 ...
                 "seed_chain"      = it$seed_chain,
                 "time"            = Sys.time() - t0),
            file = paste0("Scenario_III/Mod_v3/",it$var_lv,"/",obj_name))
