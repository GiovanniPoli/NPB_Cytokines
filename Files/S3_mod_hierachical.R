...
remove(xi_mean,xi_var,xi_par, tau_max,lambda_max, sigma_max, pi_par)


settings = c( lapply(1101:1200, function(x) list("seed_data" = as.numeric(x), "var_lv" =  "low" ,"seed_chain"= 310000  + x, "min" = 0.25, "max"= 0.75, "pi" = "Unif")),
              lapply(1201:1300, function(x) list("seed_data" = as.numeric(x), "var_lv" = "mid" ,"seed_chain"= 320000  + x, "min" = 0.75, "max"= 1.50, "pi" = "Unif")),
              lapply(1301:1400, function(x) list("seed_data" = as.numeric(x), "var_lv" = "high","seed_chain"= 330000  + x, "min" = 1.50, "max"= 3.00, "pi" = "Unif")),
              lapply(1101:1200, function(x) list("seed_data" = as.numeric(x), "var_lv" = "low" ,"seed_chain"= 340000  + x, "min" = 0.25, "max"= 0.75, "pi" = "FPR")),
              lapply(1201:1300, function(x) list("seed_data" = as.numeric(x), "var_lv" = "mid" ,"seed_chain"= 350000  + x, "min" = 0.75, "max"= 1.50, "pi" = "FPR")),
              lapply(1301:1400, function(x) list("seed_data" = as.numeric(x), "var_lv" = "high","seed_chain"= 360000  + x, "min" = 1.50, "max"= 3.00, "pi" = "FPR")))
...
foreach(it = settings, .options.snow = opts, .packages = c("lme4","reshape2","sn")) %dopar%{
  
  SIMULATION = simulate_data_heteroscedastic_no_groups_skewed(sd_min = it$min, sd_max = it$max, seed = it$seed_data, delta_par = 99/100)
  
  data = SIMULATION$simulated_data
  
  obj_name = paste0("sim_s3_singleton_hierachical_",it$var_lv, "_",it$seed_data,".rds")
  ...
  beta_start = t(sapply(paste0("comp.",1:30), function(x){
    tapply(data$value[data$variable    %in% x & data$state == "diseased"] -
             data$value[data$variable  %in% x & data$state == "healthy"],
           data$location[data$variable %in% x & data$state == "healthy"],
           mean)},  USE.NAMES = TRUE, simplify = TRUE))
  ...
  starting_values = list( beta       =  beta_start,
  ...
  if(it$pi =="Unif"){
    ...
    t0 = Sys.time() 
    chain_to_save = cytokines.model_gibbs.sampler_hierarchical(data = data, PRIORS = PRIORS, starting_values = starting_values,
    ...
    saveRDS(list("PRIORS"          = PRIORS,
                 ...
                 "time"            = Sys.time() - t0),
            file = paste0("Scenario_III/Mod_hierarchical/",it$var_lv,"/",obj_name))
  }else{
    ...
    saveRDS(list("PRIORS"          = PRIORS,
                 ...
                 "seed_chain"      = it$seed_chain,
                 "time"            = Sys.time() - t0),
            file = paste0("Scenario_III/Mod_hierarchical_ExFDR/",it$var_lv,"/",obj_name))
  }
