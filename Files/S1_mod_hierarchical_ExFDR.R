...
settings = c(lapply(  1:100, function(x) list("seed_data" = as.numeric(x), "var_lv" = "low" ,  "seed_chain"= 10000+x    , "min" = 0.25, "max"= 0.75)),
             lapply(101:200, function(x) list("seed_data" = as.numeric(x), "var_lv" = "mid" ,  "seed_chain"= 11000+x-100, "min" = 0.75, "max"= 1.50)),
             lapply(201:300, function(x) list("seed_data" = as.numeric(x), "var_lv" = "high" , "seed_chain"= 12000+x-200, "min" = 1.50, "max"= 3.00)))
...
foreach(it = settings, .options.snow = opts, .packages = c("lme4","reshape2")) %dopar%{
  ...
  saveRDS(list("PRIORS"          = PRIORS,
               ...
               "time"            = Sys.time() - t0),
          file = paste0("Scenario_I/Mod_hierarchical_ExFDR/",it$var_lv,"/",obj_name))
