...
settings = c(
  lapply(501:600, function(x) list("seed_data" = as.numeric(x), "var_lv" = "low",  "seed_chain"=   75000+x-500,
                                   "min" = 0.25, "max"= 0.75,    "eff.min" = .1, "eff.max" = 1)),
  lapply(601:700, function(x) list("seed_data" = as.numeric(x), "var_lv" = "mid", "seed_chain"=  76000+x-600,
                                   "min" = 0.75, "max"= 1.50,    "eff.min" = .1, "eff.max" = 1)),
  lapply(701:800, function(x) list("seed_data" = as.numeric(x), "var_lv" = "high",   "seed_chain"=  77000+x-700,
                                   "min" = 1.50, "max"= 3.00,   "eff.min" = .1, "eff.max" = 1)),
  lapply(801:900, function(x) list("seed_data" = as.numeric(x), "var_lv" = "low",   "seed_chain"=  78000+x-800,
                                   "min" = 0.25, "max"= 0.75,   "eff.min" = 0, "eff.max" = 0)),
  lapply(901:1000, function(x) list("seed_data" = as.numeric(x), "var_lv" = "mid", "seed_chain"= 79000+x-900,
                                   "min" = 0.75, "max"= 1.50,   "eff.min" = 0, "eff.max" = 0)),
  lapply(1001:1100, function(x) list("seed_data" = as.numeric(x), "var_lv" = "high", "seed_chain"= 710000+x-1000,
...
foreach(it = settings, .options.snow = opts, .packages = c("lme4","reshape2")) %dopar%{
  ...
  if(it$eff.max!=0){
    ...
    saveRDS(list("PRIORS"          = PRIORS,
                 ...
                 "time"            = Sys.time() - t0),
            file = paste0("Scenario_II/Mod_v3/",it$var_lv,"/sim_s2",obj_name))
  }else{
    saveRDS(list("PRIORS"          = PRIORS,
                 "start"           = starting_values,
                 "data"            = data,
                 ...
                 "seed_chain"      = it$seed_chain,
                 "time"            = Sys.time() - t0),
            file = paste0("Scenario_IV/Mod_v3/",it$var_lv,"/sim_s4",obj_name))      
  }
